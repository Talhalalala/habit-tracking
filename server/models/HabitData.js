		...
		return new Promise(async (res, rej) => {
			try {
				let results = await db.query(
					"SELECT habits.*, habit_data.* FROM habits INNER JOIN habit_data ON habits.habit_id = habit_data.habit_id WHERE habits.user_id = $1 AND habit_data.habit_date = CURRENT_DATE;",
					[user_id]
				);
				res(results.rows);
			} catch (err) {
